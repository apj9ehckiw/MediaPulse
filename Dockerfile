...
# go.mod/go.sum 在仓库根（module github.com/apj9ehckiw/mediapulse）
COPY go.mod go.sum ./
RUN go mod download
COPY backend/ ./backend/
COPY --from=frontend /dist/ backend/web/dist/
RUN CGO_ENABLED=0 GOOS=linux go build -trimpath -ldflags="-s -w" -o /out/mediapulse ./backend/cmd/server

# ---------- 阶段 3: 运行镜像 ----------
FROM alpine:3.20
RUN apk add --no-cache ffmpeg ca-certificates tzdata
WORKDIR /data
COPY --from=backend /out/mediapulse /usr/local/bin/mediapulse
